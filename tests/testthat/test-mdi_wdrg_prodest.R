...
make_panel <- function(n_firms = 50, n_periods = 5) {
  set.seed(13)
  n <- n_firms * n_periods
  data.table(
...
test_that("returns a data.table with expected columns", {
  skip_on_cran()
  result <- mdi_wdrg_prodest(make_panel(), y = "y", endog = "l", exog = "k",
                         instr = "m", id = "id", time = "year", degree = 2,
                         TFP_demeaned = FALSE)
  expect_s3_class(result, "data.table")
  expect_true(all(c("id", "year", "tfp", "el_l", "el_k",
                     "se_el_l", "se_el_k", "NumObs") %in% names(result)))
})
...
test_that("error on non-data.table input", {
  expect_error(
    mdi_wdrg_prodest(as.data.frame(make_panel()), y = "y", endog = "l",
...
test_that("error on invalid TFP_minuend", {
  expect_error(
    mdi_wdrg_prodest(make_panel(), y = "y", endog = "l", exog = "k",
                 instr = "m", id = "id", time = "year",
                 TFP_minuend = "Phi"),
    "'TFP_minuend' must be"
  )
})