...
test_that("returns a data.table with expected columns", {
  skip_on_cran()
  result <- mdi_lp_prodest(make_panel(), y = "y", endog = "l", exog = "k",
...
test_that("TFP_minuend = 'Phi' runs without error", {
  skip_on_cran()
  result <- mdi_lp_prodest(make_panel(), y = "y", endog = "l", exog = "k",
                       instr = "m", id = "id", time = "year", degree = 2,
                       TFP_minuend = "Phi", TFP_demeaned = FALSE,
...
test_that("error on non-data.table input", {
  expect_error(
    mdi_lp_prodest(as.data.frame(make_panel()), y = "y", endog = "l", exog = "k",
...
test_that("error on invalid spec", {
  expect_error(
    mdi_lp_prodest(make_panel(), y = "y", endog = "l", exog = "k", instr = "m",
